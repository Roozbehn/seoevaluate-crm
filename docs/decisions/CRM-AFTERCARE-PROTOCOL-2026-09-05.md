...
**Status: awaiting the Kaş Ekimi Uzmanı's approval.** Until the box "Standart bakım takvimi (v2) onaylandı" is ticked in Ayarlar → Süreç ayarları → Klinik, no instruction below is sent to any patient; every step opens a staff task instead. The same calendar is published on the website recovery page (`/tr|en|fa|ar/aftercare` (dedicated page)) as a `reviewRequired` section, i.e. kept out of search metadata until this approval.
