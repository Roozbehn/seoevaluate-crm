    ...
    <?php foreach ($rows as $r) {
      ...
      <tr>
        <td><code><?php echo html_escape($r['logical_name']); ?></code><br /><small class="text-muted"><?php echo html_escape($r['language']); ?> · v<?php echo (int) $r['content_version']; ?> · <?php echo html_escape($r['meta_name']); ?></small>
            ...
            <?php $btns = json_decode((string) ($r['buttons_json'] ?? ''), true); if (is_array($btns) && $btns) { ?><br /><small class="text-muted"><?php echo html_escape(_l('se_journey_template_buttons')); ?>: <?php echo html_escape(implode(' | ', array_map(function ($b) { return (string) ($b['text'] ?? ''); }, $btns))); ?></small><?php } ?></details>
